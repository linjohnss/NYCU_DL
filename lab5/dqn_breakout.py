...
class DQN:
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        with torch.no_grad():
            state = torch.Tensor(state).unsqueeze(0).permute(0, 3, 1, 2).to(self.device) # (batch_size, channels, height, width)
            actions = self._behavior_net(state)
            _, best_action = torch.max(actions, 1)
    ...
    def _update_behavior_network(self, gamma):
        ...
        with torch.no_grad():
            next_q_value = torch.max(self._target_net(next_state), 1)[0].view(-1, 1)
            target = reward + gamma * next_q_value * (1.0 - done)
...
def main():
    ...
    parser.add_argument('--episode', default=35000, type=int)
    ...
    parser.add_argument('-tmp', '--test_model_path', default='ckpt/dqn_breakout_11600000.pt')
