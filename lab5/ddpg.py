...
class DDPG:
    ...
    def select_action(self, state, noise=True):
        ...
        with torch.no_grad():
            ...
            exp_noise = torch.Tensor(sample_noise).view(1, -1).to(self.device)
            if noise:
                action += exp_noise
    ...
    def _update_behavior_network(self, gamma):
        ...
        with torch.no_grad():
           ...
           next_q_values = target_critic_net(next_state, a_next)
           target =  reward + (1.0 - done) * gamma * next_q_values
        criterion = nn.MSELoss()(q_value, target)
    ...
    for n_episode, seed in enumerate(seeds):
        ...
        while True:
            ...
            if done:
                ...
                print('Episode: {}\tReward: {:.2f}'.format(n_episode, total_reward))
                break 
